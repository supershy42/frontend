...
class FiberNode {
  ...
  getNameSpace() {
    if (this.type === 'svg') {
      if ('xmlns' in this.props) return this.props.xmlns;
      return null;
    }
    if (this.parent) return this.parent.getNameSpace();
    return null;
  }
  setKey(key) {
    this.key = key;
  ...
  parentsSiblings() {
    ...
    this.children.forEach((child, idx) => {
      child.parent = this;
      child.key = idx;
      if (child instanceof FiberNode) child.parentsSiblings();
    });
  }
  resolveFunc(Supereact) {
    this.stId = 0;
    Supereact._currentNode = this;
    const children = this.type(this.props);
    Supereact._currentNode = null;
  ...
  /**
   * @param {SReact} Supereact
   */
  reconcile(Supereact) {
    Supereact._root.parentsSiblings();
    this.parentsSiblings();
    this.old?.parentsSiblings();

    let oldChild = null;
    if (this.old && this.old.child) oldChild = this.old.child;
    ...
    while (i < children.length) {
      ...
      if (oldChild && !sameType) {
        ...
        Supereact._deletions.push(oldChild);
      }
      if (oldChild) {
  ...
  update(Supereact) {
    if (this.type instanceof Function) this.resolveFunc(Supereact);
    else if (!this.dom) this.createDom();
    this.reconcile(Supereact);
  }
  createDom() {
    if (this.type == 'TEXT_ELEMENT') this.dom = document.createTextNode('');
    else {
  ...
  updateDom = () => {
    ...
    Object.keys(oldProps)
      .filter(isEvent)
      .filter((key) => !(key in this.props) || isNew(oldProps, this.props)(key))
      .forEach((name) => {
...
class SReact {
  ...
  _printTree(node) {
    ...
    console.log(
      'T',
      node.type instanceof Function ? node.type.name : node.type,
      '|',
      ...(node.states || []),
      '|',
      ...(node?.old?.states || [])
    ...
    if (node.children) {
      for (let child of node.children) {
        if (child) this._printTree(child);
      }
    }
  }
  /** @private */
  async _renderLoop() {
    ...
    if (!this._nextTask) this._setNextTask();
  ...
  _draw_tree(dom_node, tree_node, pos = -1, onclick, bg) {
    if (!dom_node) {
      ...
      for (let el of dom_node.childNodes) dom_node.removeChild(el);
    }
    if (!tree_node) tree_node = this._root;
    const new_node = document.createElement('div');
    new_node.style.textAlign = 'center';
    new_node.style.border = '1px solid red';
    new_node.style.width = '100%';
    if (bg) new_node.style.backgroundColor = bg;
    else if (tree_node.old) new_node.style.backgroundColor = '#101055';
    else new_node.style.backgroundColor = '#101010';
    const text = document.createTextNode(
      tree_node.type instanceof Function
        ? tree_node.type.name
        : tree_node.type === 'TEXT_ELEMENT'
        ? 'T'
        : tree_node.type
    );
    ...
    if (pos < 0) dom_node.appendChild(new_node);
    else dom_node.insertBefore(new_node, Array.from(dom_node.childNodes)[pos]);
    if (tree_node.children) {
      for (let child of tree_node.children) this._draw_tree(container, child);
    }
  }
}
const Supereact = new SReact();
